...
export default function PosWorkspace() {
  ...
  const selectSection = (label: string) => { setActiveSection(label); setNavOpen(false) }
  ...
  return <main className={`pos-app ${sidebarCollapsed ? 'sidebar-collapsed' : ''}`}>
    ...
    <nav className="mobile-bottom-nav" aria-label="Navigasi cepat"><button type="button" className={activeSection === 'Kasir' ? 'mobile-nav-active' : ''} onClick={() => selectSection('Kasir')}><ShoppingCart size={19} /><span>Kasir</span></button><button type="button" className={activeSection === 'Transaksi' ? 'mobile-nav-active' : ''} onClick={() => selectSection('Transaksi')}><Receipt size={19} /><span>Transaksi</span></button><button type="button" className={activeSection === 'Produk' ? 'mobile-nav-active' : ''} onClick={() => selectSection('Produk')}><Package size={19} /><span>Produk</span></button><button type="button" className={navOpen ? 'mobile-nav-active' : ''} onClick={() => setNavOpen(true)}><Menu size={19} /><span>Menu</span></button></nav>
